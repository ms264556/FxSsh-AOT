...
namespace FxSsh.Algorithms
{
    ...
    /// it through EncryptionAlgorithm.EncryptAeadPacket / DecryptPacketLength /
    /// DecryptAeadPacket, which keeps the packet framing (length field handling,
    ...
    /// bytes - both expose the same shape here (GCM via Encrypt/Decrypt
    /// wrappers, chacha via the sequence-number nonce).
    ///
    /// This is the AEAD extension point for the SupportedAlgorithms plug-in
    /// story: a consumer (e.g. FxSsh.Tests) can register a cipher whose
    /// CipherInfo factory builds a custom IAeadTransform from the
    /// KEX-derived key without forking the library.
    /// </summary>
