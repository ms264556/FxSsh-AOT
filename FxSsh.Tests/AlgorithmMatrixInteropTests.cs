...
using X25519Kex = FxSsh.Algorithms.X25519Kex;

namespace FxSsh.Tests;

/// <summary>
/// The full algorithm matrix: EVERY algorithm the server can negotiate -
/// the built-in default set (FxSsh/SupportedAlgorithms.cs) plus every
...
public class AlgorithmMatrixInteropTests
{
    ...
    // SupportedAlgorithms; plugin rows are registered on the server by name
...
#pragma warning disable CS0618
    ...
    {
        // ---- key exchange ----
        // curve25519-sha256 and mlkem768x25519-sha256 are built in on upstream
        // dev (AlgorithmRegistry), so they need no plugin registration. The
        // libssh.org alias still has to be mapped onto the native X25519 KEX.
        ["curve25519-sha256@libssh.org"] = s => s.HazMat.KeyExchange.Add("curve25519-sha256@libssh.org", () => new X25519Kex()),
        ["diffie-hellman-group14-sha1"] = s => s.HazMat.KeyExchange.Add("diffie-hellman-group14-sha1", () => new LegacyDiffieHellmanKex()),
        ["sntrup761x25519-sha512@openssh.com"] = s => s.HazMat.KeyExchange.Add("sntrup761x25519-sha512@openssh.com", () => new Sntrup761X25519Kex()),
        ["sntrup761x25519-sha512"] = s => s.HazMat.KeyExchange.Add("sntrup761x25519-sha512", () => new Sntrup761X25519Kex()),

        // ---- host key ----
        ["ssh-ed25519"] = s =>
        {
            s.HazMat.PublicKey.Add("ssh-ed25519", key => new Ed25519Key(key));
            s.AddHostKey("ssh-ed25519", Ed25519Key.GenerateKeyPem());
        },
        ["ssh-rsa"] = s => s.HazMat.PublicKey.Add("ssh-rsa", x => new LegacyRsaKey(x)),

        // ---- cipher ----
        ["chacha20-poly1305@openssh.com"] = s => s.HazMat.Encryption.Add("chacha20-poly1305@openssh.com",
            () => new CipherInfo(key => new ChaCha20Poly1305Transform(key), keySize: 512, blockSizeBits: 64)),
        ["aes128-ctr"] = s => s.HazMat.Encryption.Add("aes128-ctr", () => new CipherInfo(Aes.Create(), 128, CipherModeEx.CTR)),
        ["aes192-ctr"] = s => s.HazMat.Encryption.Add("aes192-ctr", () => new CipherInfo(Aes.Create(), 192, CipherModeEx.CTR)),

        // ---- MAC ----
        ["hmac-sha1"] = s => s.HazMat.Hmac.Add("hmac-sha1", () => new HmacInfo(new HMACSHA1(), 160)),
        ["hmac-sha1-etm@openssh.com"] = s => s.HazMat.Hmac.Add("hmac-sha1-etm@openssh.com", () => new HmacInfo(new HMACSHA1(), 160, isEtm: true)),
        ["umac-64@openssh.com"] = s => s.HazMat.Hmac.Add("umac-64@openssh.com",
            () => new HmacInfo(key => new UmacHmacAlgorithm(key, 8), keySize: 128, isEtm: false)),
        ["umac-64-etm@openssh.com"] = s => s.HazMat.Hmac.Add("umac-64-etm@openssh.com",
            () => new HmacInfo(key => new UmacHmacAlgorithm(key, 8), keySize: 128, isEtm: true)),
        ["umac-128@openssh.com"] = s => s.HazMat.Hmac.Add("umac-128@openssh.com",
            () => new HmacInfo(key => new UmacHmacAlgorithm(key, 16), keySize: 128, isEtm: false)),
        ["umac-128-etm@openssh.com"] = s => s.HazMat.Hmac.Add("umac-128-etm@openssh.com",
